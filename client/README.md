...
- Leaf-label rename, tip-folder creation, backup, and rollback are available directly in Settings. The editor uses the Python standard library to patch the original tree/NEXUS text; it does not launch Synology Text Editor or any external program. A write is only made after its preview and the explicit `Confirm and write back` action.
