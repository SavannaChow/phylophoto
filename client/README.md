...
- Leaf-label rename and tip-folder creation are available directly in Settings. The editor uses the Python standard library to patch the original tree/NEXUS text; it does not launch Synology Text Editor or any external program. Applying selected renames writes immediately and appends the changes to `rename-history.txt` beside the tree.
- Rename only targets leaf labels. Internal labels, bootstrap/support values, branch lengths, annotations, whitespace outside the replaced token, and topology are not regenerated. Matching tip photo folders are renamed in the same operation.
